...
class Owner(db.Model, UserMixin):
    ...
    is_verified = db.Column(db.Boolean, default=False)# if admin verified the owner
...
class OperatingHours(db.Model):
    ...
    opening_time = db.Column(db.String(8), nullable=False)  # Format: 'HH:MM'
    closing_time = db.Column(db.String(8), nullable=False)  # Format: 'HH:MM'
...
class Service(db.Model):
    ...
    id = db.Column(db.Integer, primary_key=True)
    category = db.Column(db.String(50), nullable=False, index=True)# Massage, Facial, etc.
...
class SpaImage(db.Model):
    ...
    def create_from_upload(cls, file, spa_id: int, upload_folder: str, is_primary: bool = False) -> Optional['SpaImage']:
        ...
        try:
            ...
            current_app.logger.error(f"Failed to save uploaded image: {str(e)}")# for debugging
            return None
...
class ReviewImage(db.Model):
    ...
    @classmethod
    def create_from_upload(image_record, file, review_id, upload_folder):
